...
const Home: NextPage = ({ posts }: any) => {
  ...
  useEffect(() => {
    getCategoryPost("").then((result) => setGridItems(result));
  }, []);

  return (
    <>
      <NextSeo title='Home | Bentech.' />
                  ...
                  gridItems?.map((gridItem: any, i: number) => (
                    <div className='lg:w-1/2' key={gridItem.node.title}>
                      <div
                        ...
                        <Link href={`/post/${gridItem.node.slug}`}>
                          <h1 className='pl-3 text-xl text-gray-800 font-semibold cursor-pointer hover:opacity-60 transition ease-in-out duration-300'>
                            {gridItem.node.title}
                          </h1>
                        </Link>
